...
- Сообщения и описания git-коммитов (commit messages) всегда писать только на английском языке.
- **Всегда обновлять документацию и changelog при коммитах**: любые изменения (новые фичи, фиксы, изменения конфигурации или поведения) обязательно вносить в `CHANGELOG.md` (RU) и `CHANGELOG.en.md` (EN) в секцию `[Unreleased]`, а также синхронно актуализировать всю затронутую документацию (`README.md`, `README.en.md`, `SMOKE.md`, `SMOKE.en.md`, `smoke-stand/README.md`, `capabilities.yaml`, примеры конфигурации) до или вместе с коммитом.
- **Очистка target/ и обновление AST-индексов после пуша**: сразу после выполнения `git push` всегда очищать директорию артефактов сборки `target/` (`mvn clean` или удаление папки `target/`), освобождая дисковое пространство, и сразу после этого пересобирать AST-индексы проекта командой `./scripts/build-java-ast-index.sh`, чтобы они всегда были актуальны для последующей навигации.
...
- Кэширование списков баз (`DatabaseListCache`, `routing.database-list-cache.*`) и метаданных баз (`DatabaseMetadataCache`, `routing.database-metadata-cache.*`) изолирует сессионный пул бэкенда от всплесков однотипных запросов (`SHOW DATABASES` / `get_database` от HiveServer2 и Ranger). Оба кэша реализуют SingleFlight (дедупликация параллельных запросов в полете), отдают только глубокие копии (`new Database(cached)` для структур), и автоматически инвалидируются при выполнении DDL-мутаций (`create_database`, `alter_database`, `drop_database`).
...
- **Вывод содержит не только результаты.** `beeline -f` печатает каждый выполняемый statement, а HiveServer2 логирует его ещё дважды («Compiling command», «Executing command»), поэтому маркер-литерал из SQL — ветка `case ... then 'ok' else 'bad'` или вставляемое значение — лежит в файле вывода независимо от того, что запрос на самом деле вернул. Измерено: на зелёном прогоне «плохие» маркеры (`convert_e2m_left_*`, `msck_present_before_*`) встречаются по три раза как подстрока и ни разу как результат. Проверяй такие маркеры только с якорем на начало строки (`assert_file_contains_result`).
