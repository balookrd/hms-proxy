...
- Перед анализом или доработкой Java-кода используй AST-индекс проекта, чтобы сначала найти нужные классы, методы, вызовы и тесты, а уже потом читать полные файлы. Если индекс отсутствует или мог устареть после изменений в `src/main/java` или `src/test/java`, перегенерируй его командой `scripts/build-java-ast-index.sh`.
- Индексатор пишет generated artifacts в `target/java-ast-index/`: `java-symbols.json`, `symbols.tsv`, `classes.tsv`, `methods.tsv`, `calls.tsv`, `news.tsv`, `affected-tests.tsv`. Предпочитай `scripts/java-index-query.sh` для типовых запросов (`class`, `method`, `call`, `new`, `tests`); для нестандартных запросов используй `rg` по TSV-срезам. Исходники открывай точечно по найденным путям и строкам.
