...
- `smoke-stand/` - локальный docker-compose стенд для тех же smoke-скриптов: два standalone metastore (Apache и Hortonworks) за прокси, HDFS, HiveServer2 и MIT KDC. Собирается из jar-файлов каталога `hive-metastore/`; тяжёлые входные артефакты не хранятся в git и восстанавливаются через `smoke-stand/prepare.sh`. Профиль `hive4` (`.env.hive4[-kerberos]`) подменяет default-каталог на метастор Apache Hive 4.1.0 (`smoke-stand/hms-hive4/`, официальный образ) и добавляет Iceberg interop-сценарий `smoke-stand/run-iceberg-interop-smoke.sh`: REST-клиент `smoke-stand/iceberg-rest-writer/` пишет настоящие данные через REST front door, а оба 3.1-HiveServer2 (с `iceberg-hive-runtime` 1.6.1) и HiveServer2 Hive 4 (`smoke-stand/hs2-hive4/`, через третий listener `additional-frontends.hive4fe` с профилем `APACHE_4_1_0`) читают и дописывают ту же таблицу. Детали и известные ограничения - в `smoke-stand/README.md` и секции H `smoke-stand/TEST-MATRIX.md`.
