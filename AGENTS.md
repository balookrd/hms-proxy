...
- **Очистка target/ после пуша**: сразу после выполнения `git push` всегда очищать директорию артефактов сборки `target/` (`mvn clean` или удаление папки `target/`), освобождая дисковое пространство.
