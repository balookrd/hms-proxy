...
- `restcatalog` - Iceberg REST front door поверх routing-слоя: per-catalog сервисы работают через in-process прокси `IMetaStoreClient`, доступ только на чтение, для не-default каталогов включён name translation, чтобы клиент видел внутренние имена баз этого каталога.
