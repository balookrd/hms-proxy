...
- **Проверка обязана уметь падать, и это надо увидеть.** Прежде чем засчитывать зелёный прогон, сломай проверяемое и убедись, что он краснеет. Вакуумные формы, пойманные на практике: `select count(*)` вместо выборки значения (печатает число при любом содержимом); `TRUNCATE`, признанный успешным по отсутствию ошибки, тогда как Hortonworks-клиент шлёт пустой список партиций и не усекает ничего; ассерт на строку, которой в выводе не бывает вовсе.
