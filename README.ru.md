...
При включённой имперсонации прокси устанавливает Kerberos/SASL соединение с бэкендом HMS под
собственным клиентским Kerberos-принципалом (`security.client-principal` с keytab), а для каждого
пользователя держит отдельную сессию (`ImpersonationClient`), в которой при инициализации выполняет
`set_ugi(user, groups)`. Серверный обработчик метастора (`TUGIBasedProcessor`) привязывает переданный UGI
пользователя к сессии, и все последующие операции (такие как `wh.mkdirs` в HDFS при создании таблиц)
выполняются от имени вызывающего пользователя, а не `hive:hadoop`.
