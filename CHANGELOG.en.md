...
- **Fix Hive pattern matching in `CatalogRouter` for ODBC/JDBC clients (Qlik, Simba ODBC)**:
  - Fixed an issue in `CatalogRouter.compileHivePattern` where ODBC clients (notably Qlik via Simba ODBC) failed to see schemas and tables containing underscores `_` in their names (e.g. `edo_reporting_dm`, `domain_edo_beedocs_dm`, `bt__crm`).
  - When processing SQL LIKE requests from drivers, HiveServer2 (`MetadataOperation.convertPattern`) translates single-character wildcards `_` into dots `.` (regex single-character wildcard). Previously, `compileHivePattern` escaped `.` as a literal dot `\.`, which caused post-filtering in `matchesHivePattern` to purge all schemas and tables matching the pattern.
  - The `.` character in `compileHivePattern` is now retained as an unescaped regex single-character wildcard `.` (matching native Hive Metastore `ObjectStore` behavior), enabling proper matching of dot-separated patterns against underscore-containing schema and table names.
