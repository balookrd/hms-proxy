...
- **Регламент очистки target/ после пуша**:
  - В `AGENTS.md` и памяти проекта зафиксировано правило обязательной очистки каталога артефактов сборки `target/` сразу после отправки коммитов (`git push`).
...
- **Регламент синхронизации документации при коммитах**:
  - В `AGENTS.md` и памяти проекта зафиксировано требование: любые изменения в кодовой базе обязательно коммитятся синхронно с обновлением документации (RU/EN) и чейнджлогов.
...
- **Маршрутизация шаблонов схем HiveServer2 / JDBC (DBeaver, Hue)**:
  - Исправлена ошибка отображения таблиц удалённых каталогов в навигаторе метаданных DBeaver, Hue и DataGrip: HiveServer2 преобразует неэкранированные подчёркивания LIKE-паттерна в точки (`convertSchemaPattern`), превращая разделитель каталогов `__` в `..`.
  - В `CatalogRouter.resolvePattern` добавлена поддержка транслированных шаблонов префиксов каталогов (включая каталоги с подчёркиваниями в имени), что обеспечивает точную маршрутизацию запросов `get_table_meta` и `get_databases` на целевой бэкенд без ложного ухода в fanout.
