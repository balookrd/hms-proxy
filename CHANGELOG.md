...
- **Фоновое автообновление кэша баз данных (Background Cache Refresh)**:
  ...
  - Реализована отказоустойчивость: при сбоях бэкенда старые закэшированные записи не затираются, клиенты продолжают получать ответы, а ошибка фиксируется в метрике `result="failure"`.
  - Экспортированы метрики `hms_proxy_cache_refreshes_total{cache, catalog, result}`, добавлены панели мониторинга в Grafana Dashboard и smoke-сценарий `smoke-stand/run-database-cache-smoke.sh` на Docker-стенде.
...
- **Имперсонация пользователей в Kerberos через Delegation Tokens**:
  ...
  - Все объекты в HDFS (каталоги баз данных, таблицы, партиции) создаются с владельцем, соответствующим реальному пользователю клиентского запроса, а не сервисному пользователю прокси.
- **Метрики памяти JVM**:
  - Добавлены метрики использования heap/non-heap памяти, пулов памяти JVM и прямых буферов (`jvm_memory_used_bytes`, `jvm_memory_committed_bytes`, `jvm_buffer_pool_used_bytes`).
  - В Grafana dashboard добавлена секция мониторинга использования ресурсов JVM.
- **Инструменты AST-индексации проекта**:
  - Добавлены инструменты `scripts/build-java-ast-index.sh` и `scripts/java-index-query.sh` на базе `ast-grep` для быстрого структурного поиска классов, методов и вызовов без перерасхода токенов контекста.
- **Документация по отказоустойчивости (`FAILOVER.md`)**:
  - Создано подробное руководство по поведению бэкендов при сбоях, таймаутах, деградированном чтении (degraded reads) и процедурам восстановления.

### Изменено

- **Ревизия качества русскоязычной документации и терминологии**:
  - Проведён полный аудит всех русскоязычных документов (`README.md`, `COMPATIBILITY.md`, `FAILOVER.md`, `SMOKE.md`, `smoke-stand/README.md`, `smoke-stand/TEST-MATRIX.md`, `capabilities.yaml`, `AGENTS.md`).
  - Устранены буквальные кальки с английского языка («внутренализуется» $\to$ «интернализуется», «говорят через...» $\to$ «использующие обёртки запросов...», «по design» $\to$ «на уровне архитектуры (by design)», «estate» $\to$ «инфраструктура»).
  - Заголовок раздела `## Три опоры` переименован в `## Три ключевых принципа`.
  - Устранены разговорные обороты и жаргонизмы («race'ят несколько metastore», «дренирует весь пул», «quiesce / квиесцировать», «залипание бэкенда», «гонять наборы», «дропнул»).
  - Исправлены грамматические и орфографические ошибки (несогласованность родов и падежей, восстановлена буква «ё»), устранено дублирование абзацев про `_dummy_database` в `README.md`.
  - Стиль обращения к читателю унифицирован к академическому и вежливому техническому стилю (устранены скачки на «ты»).
- **Смена основной локали документации**:
...
- **Стабильность smoke-скриптов стенда**:
  - В `smoke-stand/run-database-cache-smoke.sh` и `smoke-stand/run-ranger-shared-cache-smoke.sh` исправлена фильтрация вывода `get_database`, повышена отказоустойчивость суммирования Prometheus-метрик при пустом результате под `pipefail`, добавлен флаг `--force-recreate` при перезапуске контейнера прокси.

- **Маршрутизация шаблонов схем HiveServer2 / JDBC (DBeaver, Hue)**:
  - Исправлена ошибка отображения таблиц дефолтного и удалённых каталогов в навигаторе метаданных DBeaver, Hue и DataGrip:
    - HiveServer2 заменяет неэкранированные подчёркивания LIKE-паттерна в `getTables` на точки (`convertSchemaPattern`), превращая `dmp_raw` в `dmp.raw`. В `CatalogRouter.normalizeExternalDbName` устранено ложное отсечение первого сегмента для незарегистрированных префиксов каталогов (ранее `dmp.raw` ошибочно обрезалось до `raw`).
    - В `GetTableMetaHandler` и `GetDatabasesHandler` добавлена проверка `canMatchRemoteCatalogs(pattern)`: паттерны, адресованные исключительно дефолтному каталогу (например, `default`, `dmp.raw`), больше не уходят в долгий параллельный fanout по удалённым бэкендам, предотвращая задержки и таймауты (3+ секунды) при загрузке таблиц в DBeaver.
    - В `CatalogRouter.resolvePattern` поддержаны транслированные шаблоны префиксов каталогов (включая каталоги с подчёркиваниями в имени).
    - В `GetTableMetaHandler` реализована динамическая экстернализация таблиц по их фактическому имени базы данных (`result.getDbName()`), гарантирующая корректные имена баз при запросах по маскам вида `catalog..*`.
- **Устранение утечки контекста пользователя в Thrift**:
  - Привязка `UserGroupInformation` в методе `set_ugi` переведена на уровень сетевого транспорта соединения (`ClientRequestContext.setConnectionUgi`), что предотвращает утечки контекста аутентификации между рабочими потоками `TThreadPoolServer`.
- **Нормализация исключений Thrift RPC**:
  - Инфраструктурные исключения `TException` унифицированы и корректно маппятся на всех путях вызова бэкенд-адаптеров (как для прямого вызова, так и через рефлексию).
...
- **Скользящее пакетное устаревание кэшей**:
  - Реализовано скользящее пакетное устаревание (sliding batch expiration) для кэшей списка баз данных и метаданных, предотвращающее одновременное истечение записей и лавинообразную нагрузку на бэкенды.
- **Синтетический ответ для `refresh_privileges`**:
  - Добавлена настройка `routing.refresh-privileges.synthetic-success` для быстрого и безопасного ответа на частые вызовы сброса привилегий от HiveServer2/Ranger без нагрузки на бэкенд.
  - Поддержан конфигурационный алиас `ttl-seconds` для настроек времени жизни кэшей.
- **Автоматизация Release Notes**:
  - Добавлена конфигурация `cliff.toml` и интеграция с `git-cliff` для генерации списков изменений релизов из git-истории.

### Оптимизация

- **Single-Flight Coalescing и ускорение Fanout**:
  - Реализовано объединение параллельных идентичных запросов метаданных (single-flight coalescing), исключающее дублирование запросов к бэкенду.
  - Ускорено параллельное выполнение fanout-запросов (`get_all_databases`) по зарегистрированным каталогам.

### Исправлено

- Исправлена маршрутизация RPC-вызова `refresh_privileges` и корректная обработка каталогов в режиме `READ_ONLY`.
...
- **Полное покрытие операций пути данных (SQL Data Path)**:
  - Покрыты тестами на реальном стенде операции, перемещающие и изменяющие данные: `CTAS` (CREATE TABLE AS SELECT), `INSERT OVERWRITE`, `LOAD DATA` и смена типа таблицы `managed <-> external` (строка матрицы C12).
- **Покрытие `MSCK REPAIR TABLE`**:
  - Реализован smoke-сценарий проверки восстановления партиций с обходом каталогов в HDFS и их корректной регистрацией в метасторе через прокси.

## 2026-08-04

### Добавлено

- **Сборка образов бэкендов на Hadoop 3.1 и вендорском Hadoop**:
  - Образ Apache Metastore переведён на Hadoop 3.1; образ Hortonworks Metastore собирается на базе вендорского дистрибутива Hadoop.
  - Добавлен слот переопределения classpath `hms/override-{hdp,apache}` для подмены библиотек Hadoop впереди Maven-зависимостей, что устранило падения `TRUNCATE` из-за несовместимости `HdfsAdmin`.
- **Точные проверки очистки таблиц**:
  - Проверка `TRUNCATE` валидирует фактическое отсутствие данных в таблице, а не только отсутствие ошибок ответа RPC.
- **Корректные Iceberg-таблицы для линии Hive 3.1**:
  - В smoke-сценариях таблицы Iceberg для линии 3.1 создаются как `EXTERNAL TABLE`, исключая конфликты взаимных блокировок managed-таблиц под управлением `DbTxnManager`.

## 2026-08-03

### Тесты и верификация

- **Полноценная проверка SQL-слоя в Kerberos**:
  - Обе пары front door (Apache и Hortonworks) протестированы в защищённом Kerberos-окружении.
- **Парная раскладка front-door**:
  - Каждый front door тестируется со своим метастором в роли default-каталога, а также с удалённым каталогом.
  - Подтверждено сохранение Write ID транзакций при операциях через прокси (строка C6 матрицы).
