...
**На той форме, которую стенд реально шлёт, guard пока не срабатывает.** Проверено по проводу:
`alter_table` от HiveServer2 несёт `params={EXTERNAL, numFiles, numRows, totalSize,
transient_lastDdlTime}` и **никакого `metadata_location`**, поэтому guard, который смотрит на
устаревший указатель *в запросе*, выходит сразу. Шесть чистых прогонов после изменения не
доказывают ничего: WARN, который он пишет при починке указателя, не появился ни разу, а при
наблюдённой частоте потерь один к восьми серия из шести чистых прогонов случается примерно в 45%
случаев и без всякого фикса. Ориентироваться надо на то, что метастор держит **сейчас** (является
ли таблица Iceberg-овой), а не на то, что прислал клиент, — ценой чтения на каждый `alter_table`,
которое надо чем-то ограничить.
