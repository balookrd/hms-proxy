...
Всю гонку это не закрывает: guard читает текущий указатель, а бэкенд применяет alter — это два
отдельных вызова, и коммит, попавший между ними, по-прежнему затёрся бы. **Считать строку
починенной нельзя.** На Hive 4-бэкендах починенный alter теперь несёт
`expected_parameter_key`/`expected_parameter_value` HMS, что превращает это в громкий отказ вместо
тихого затирания; линия 3.1 оба ключа игнорирует, и там окно остаётся открытым. Чтобы закрыть
по-настоящему, прокси должен держать тот же табличный лок, что берёт Iceberg, на всё
чтение-и-запись.
