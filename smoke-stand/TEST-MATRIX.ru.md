...
**Почему строки `--origin` названы для бэкенда `hive4`.** На бэкенде `hdp` они не гоняются и, по
измерению от 2026-08-04, гоняться не могут: таблица, созданная там 3.1-движком, блокирует
собственный `INSERT`, и причина до прокси даже не доходит. `create table ... stored by
'HiveIcebergStorageHandler'` ложится в Hortonworks-метастор как `MANAGED_TABLE`, а запись в
managed не-ACID таблицу под `DbTxnManager` заставляет Hive взять на неё **EXCLUSIVE**-лок на всё
время запроса (`show locks` во время прогона: транзакция самого запроса держит
`default.<таблица> ACQUIRED EXCLUSIVE`). Iceberg-коммит, которым тот же самый запрос
завершается, выполняется в `HiveIcebergOutputCommitter.commitJob` и просит у метастора свой
EXCLUSIVE-лок на таблицу (`org.apache.iceberg.hive.MetastoreLock`) — то есть встаёт в очередь за
локом, который сам же и держит. Четыре попытки по три минуты, локальная MapReduce-задача умирает,
и beeline сообщает только `return code 2 from org.apache.hadoop.hive.ql.exec.mr.MapRedTask`;
настоящая причина видна лишь в логе HiveServer2 как `MetastoreLock$WaitingForLockException`.

Остальным путям достаётся другая таблица — не managed. Metadata transformer метастора Hive 4
переписывает нетранзакционную managed-таблицу во внешнюю: тот же DDL через тот же HDP
HiveServer2 даёт `Table Type: EXTERNAL_TABLE` и `TRANSLATED_TO_EXTERNAL=TRUE`. Таблица же,
созданная REST-фронтом, внешняя изначально. Поэтому H2 и прогоны `--origin rest` проходят и на
бэкенде `hdp`: Hive лочит только плейсхолдер `_dummy_database`, а Iceberg-лок выдаётся сразу.
Собственный лок pointer-guard’а в прокси здесь ни при чём: он встаёт в очередь за
EXCLUSIVE-локом запроса, сдаётся по своему бюджету в 10 секунд, снимает собственную заявку и
чинит запись без лока.
`--origin apache` на бэкенде `hdp` не измерялся; он гонит тот же DDL через тот же метастор,
поэтому та же взаимоблокировка ожидаема.
...
- **2026-08-04**, jar прокси тот же, что в записи выше: изменение целиком в двух образах
  ...
  плейсхолдер `_dummy_database`, и запрос проходит. См. «Почему строки `--origin` названы для
  бэкенда `hive4`» в секции H.
