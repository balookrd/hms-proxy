...
**Граница проходит по записи, а не по чтению** — прямо противоположно тому, чего можно было бы
ждать по асимметрии с `inputFormat` выше. HiveServer2 3.1 с `iceberg-hive-runtime` 1.6.1 планирует
скан таблицы format-version 2 с position-deletes и применяет их; чего он не умеет, так это их
*порождать*: storage handler Hive 3 не регистрирует таблицу как транзакционную, и семантический
анализатор останавливает запрос до того, как появится план. То есть писатель Hive 4 и читатель 3.1
могут работать с одной и той же построчно изменённой таблицей, а клиент 3.1, который попробует её
изменить, падает сразу и заметно, а не дописывает половину. Как и в H12, решения прокси здесь нет:
`alter_table` прокси ретранслирует в обе стороны одинаково.
...
| I6 | Тот же маршрут **не** атомарен, когда падает не требование, а сам коммит: все требования проверяются заранее, после чего таблицы коммитятся по одной без откатов, поэтому сбой на середине оставляет предыдущие таблицы закоммиченными, а прокси отвечает `500 CommitStateUnknownException` (`IcebergRestEndpointIntegrationTest#multiTableTransactionMustNotReportSuccessWhenTheSecondCommitFails`, подтверждено на стенде, на живом метасторе Hive 4, исчерпанием ddl-класса rate-limit'а) | n/a | ✅ |
