...
| I4 | **Через разные front door**: REST-append'ы и Hive-`INSERT`'ы (Hortonworks front door) коммитят в одну таблицу с пересекающимися окнами коммита — 13 писателей из 14 коммитят, один REST-писатель отклонён, в таблице ровно 14 строк. Две реализации Iceberg, никогда не делящие JVM (1.9.2 внутри прокси и `iceberg-hive-runtime` 1.6.1 внутри HiveServer2), сериализуются друг относительно друга через метастор | ✅ | — |
