...
| C10 | `TRUNCATE` в **метастор Hortonworks как удалённый каталог** отклоняет сам этот метастор, а не прокси: на позиционный `truncate_table` он вызывает `HdfsAdmin.getEncryptionZoneForPath`, которого в подходящей сигнатуре на его classpath нет, падает с `NoSuchMethodError`, рвёт соединение, и клиент видит голый `TTransportException` за упавшим `DDLTask`. Тот же оператор через Hortonworks front door проходит, то есть вендорский путь `truncate_table_req` этого вызова не делает. Управляется `HMS_SMOKE_SQL_RUN_TRUNCATE`, выключен в `sql-apache-kerberos.env` | — | ✅ воспроизведено |
