...
| `CTAS`, `INSERT OVERWRITE`, `LOAD DATA`, конвертация managed↔external | Через прокси не гонялись ни разу. C9 покрывает `ADD COLUMNS`, `RENAME TO`, `DROP PARTITION` и `TRUNCATE`, C11 — `MSCK REPAIR`; это следующий слой |
