...
Какие именно проверки прогонялись на этом профиле — раздел G в `TEST-MATRIX.md`.
...
что сломанная Avro-зависимость видна только здесь. Эта проверка читает HDFS изнутри контейнера
namenode, у которого под Kerberos нет своего тикета, поэтому сценарий сначала логинит его по
keytab узла: без этого вызов падает с `Client cannot authenticate via:[TOKEN, KERBEROS]`, и
нечитаемый HDFS выглядел бы пустым (так и было до 2026-08-04 — см. журнал ревалидаций).
Если сразу после пересборки стенда
...
namenode. Что именно прогонялось — раздел H в `TEST-MATRIX.md`.
...
[Error 10297] ... that is not transactional`. Подробности — H13-H20 в `TEST-MATRIX.md`.
...
берёт настоящий Hive-лок; остальные каталоги обслуживает synthetic shim, выдающий локи без
проверки конфликтов. Этот сценарий проверяет ту половину утверждения, которую стенд проверить
может:
...
Он создаёт таблицу с одной базовой строкой, запускает N конкурентных дописываний через REST
front door, считает писателей, завершившихся с кодом 0, и требует, чтобы в таблице оказалось
ровно столько строк плюс базовая. Писатель, упавший громко (`CommitFailedException`), — это
корректное поведение под состязанием, и прогон он не валит; писатель, отчитавшийся об успехе, но
без своих строк, — валит. На восьми писателях стенд обычно даёт оба исхода сразу: семь коммитов и
один отказ, причём все закоммиченные строки на месте. Но не всегда — исчерпает ли кто-нибудь из
писателей свои retry, меняется от прогона к прогону, и прогон, где закоммитили все восемь, ровно
так же корректен. Сценарий проверяет один инвариант: число строк равно числу писателей,
отчитавшихся об успехе.

`--sql-writers N` добавляет Hive-`INSERT`'ы в ту же таблицу, и состязание становится межпутевым:

```bash
smoke-stand/run-iceberg-concurrency-smoke.sh --prefix hive4 --writers 6 --sql-writers 2 --sql-engine hdp
```

Beeline-`INSERT` десятки секунд занят MapReduce до коммита, а REST-append коммитит сразу, поэтому
одновременный запуск просто выстроил бы их в очередь и ничего не доказал. Вместо этого сценарий
шлёт REST-append'ы раундами, пока жив хоть один SQL-писатель, а потом проверяет, что окна
коммитов действительно пересеклись: он вычитывает время `alter_table` из лога прокси, где пути
различаются по имени потока. См. секцию I в `TEST-MATRIX.md`.

## Multi-table транзакции под состязанием (`run-iceberg-txn-contention-smoke.sh`)

`POST /v1/{prefix}/transactions/commit` несёт в одном запросе несколько таблиц, и клиент вправе
прочитать это как «всё или ничего». Сценарий измеряет, что происходит на самом деле, когда
конкурирующий писатель уводит одну из этих таблиц из-под транзакции:

```bash
smoke-stand/run-iceberg-txn-contention-smoke.sh --prefix hive4 --kerberos
```

Состязание здесь настоящее, а не подстроенное: второй писатель дописывает одну из двух таблиц
через тот же front door, продвигая её ссылку `main`, и транзакция приходит уже со snapshot id,
прочитанным до этого append'а, — именно то, что отправил бы проигравший гонку, поэтому играть с
таймингами не нужно. Транзакция отклоняется с `409 CommitFailedException: Requirement failed:
branch main has changed`, изменение не остаётся ни на одной из таблиц, а строки конкурента целы.

Затем прогон выполняет ту же транзакцию с актуальным snapshot id и требует, чтобы она была
**принята** и применена к обеим таблицам. В этом позитивном контроле всё дело: без него отказ выше
выглядел бы столь же убедительно и при некорректном теле, неверном prefix или таблице, вообще
недоступной для записи.

То, что состязание по требованиям — «всё или ничего», **не** делает маршрут атомарным в общем
случае. Адаптер Iceberg проверяет все требования заранее, а затем коммитит таблицы по одной без
откатов. Поэтому, когда падает не требование, а сам коммит, сбой на середине оставляет предыдущие
таблицы закоммиченными, а прокси отвечает `500 CommitStateUnknownException`. Обе половины записаны
как I5 и I6 в `TEST-MATRIX.md`.
...
Проверяет фоновое периодическое автообновление кэша списка баз данных (`DatabaseListCache`) и метаданных баз данных (`DatabaseMetadataCache`), пока к ним есть активные обращения клиентов:
...
Сценарий тестирует жизненный цикл фонового обновления (`DatabaseCacheRefresher`) на запущенном прокси с конфигурацией `hms-proxy-db-cache.properties`:
- Создаёт тестовую базу данных `db_cache_smoke_test`.
- Делает прогревочные запросы `get_all_databases` и `get_database`, заполняя кэши и фиксируя временные метки активности.
- Проверяет инкремент метрики `hms_proxy_cache_refreshes_total` для обоих кэшей (`database_list` и `database_metadata`), подтверждая фоновый опрос бэкендов в активном окне.
- Выдерживает паузу без обращений дольше окна активности (`activity-window-ms=10000`) и проверяет засыпание: метрики обновлений перестают расти.
- Выполняет новые клиентские запросы и подтверждает пробуждение: фоновые обновления возобновляются.

## Шаблоны схем DBeaver / Hue (`run-schema-pattern-smoke.sh`)

Проверяет разрешение транслированных шаблонов схем, передаваемых JDBC GUI-клиентами (DBeaver, Hue, DataGrip) через HiveServer2:
...
Сценарий проверяет:
- `get_databases` с шаблонами `hdp..*` и `apache..*`, проверяя корректное разрешение и экстернализацию (`hdp__default`, `apache__default`).
...
## Отказоустойчивость Cross-DC и WAN (`run-cross-dc-resilience-smoke.sh`)

Проверяет комплекс механизмов отказоустойчивости при связывании удаленных датацентров и деградации WAN-каналов:
...
Сценарий тестирует на запущенном прокси с конфигурацией `hms-proxy-cross-dc.properties`:
- **Кэши метаданных таблиц и разделов (`TableMetadataCache`, `PartitionMetadataCache`)**: проверку попаданий (`hit`) и промахов (`miss`) в кэш при последовательных вызовах `get_table` и `get_partition` по счетчикам Prometheus `hms_proxy_cache_requests_total`.
- **Автоматическую DDL-инвалидацию**: проверку сброса кэшированных записей таблицы и разделов при выполнении мутаций `alter_table`.
- **Режим Serve-Stale-on-Error**: проверку выдачи устаревших метаданных из кэша (`stale_hit`) при искусственной остановке удаленного каталога (`docker stop stand-hms-apache`) с сохранением работоспособности клиентских запросов.
- **Гранулярный `/readyz`**: подтверждение HTTP `200 OK` при падении вторичного каталога с флагом `required-for-readiness=false` (локальный балансировщик нагрузки не снимает прокси с локального трафика при отказе удаленного ЦОД).
- **Catalog Shadow / Replica Fallback**: прозрачное перенаправление read-only запросов (`get_table`) на локальную теневую реплику (`hdp`) при сбое удаленного каталога с инкрементом метрики `hms_proxy_catalog_fallback_total`.
- **Защиту от Split-Brain**: гарантированное отклонение любых мутирующих операций (`create_table`, `alter_table`, `drop_table`) к отказавшему каталогу без изменения реплики.
- **Lenient Startup**: успешный запуск и инициализацию прокси при полностью выключенном удаленном вторичном бэкенде.
- **Восстановление (Recovery)**: возврат удаленного бэкенда в строй и проверку восстановления штатной маршрутизации.
