...
| C10 | `TRUNCATE` against the **Hortonworks metastore as a remote catalog** is refused by that metastore, not by the proxy: it answers a positional `truncate_table` by calling `HdfsAdmin.getEncryptionZoneForPath`, which its own classpath carries in no matching signature, dies with `NoSuchMethodError`, drops the connection, and the client sees a bare `TTransportException` behind a failed `DDLTask`. Settled by experiment: the variable is one argument. The Apache client sends `partNames=null` - truncate the whole table - so the metastore deletes the table directory and reaches the encryption-zone check that fails to link. The vendor client sends `partNames=[]`, an empty partition list, so the metastore has nothing to delete and returns without doing any work. Ruled out along the way, each by holding it constant: the catalog's role (the same client fails with `hdp` as the default catalog too) and the location (identical in both roles). **Consequence worth acting on: `TRUNCATE` through the Hortonworks client is a no-op**, so a run that only checks it does not fail proves nothing - the row count after it has to be asserted. Gated by `HMS_SMOKE_SQL_RUN_TRUNCATE`, off in `sql-apache-kerberos.env` | — | ✅ reproduced |
...
| `CTAS`, `INSERT OVERWRITE`, `LOAD DATA`, `MSCK REPAIR`, managed↔external conversion | Never run through the proxy. C9 covers `ADD COLUMNS`, `RENAME TO`, `DROP PARTITION` and `TRUNCATE`; these are the next layer |
