...
run_sql_smoke() {
  ...
  # TRUNCATE empties the whole table when partNames is null. The assertion below checks the row count
  # (truncate_emptied_*) rather than the absence of an error to ensure files are actually deleted on HDFS.
